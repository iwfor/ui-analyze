...
module UiAnalyze
  module CLI
    ...
    def self.print_disk_info(disk, az = nil)
      section "Storage"
      print_attached_disks(disk, az)
      print_filesystems(disk)
    end

    def self.print_attached_disks(disk, az = nil)
      if disk.attached_disks.any?
        disk.attached_disks.each do |d|
          print_disk_entry(d, az)
        end
      else
    ...
    def self.print_disk_entry(d, az = nil)
    ...
    def self.print_filesystems(disk)
      ...
      return unless (total = disk.swap_total_bytes)

      used = disk.swap_used_bytes || 0
      free = disk.swap_free_bytes || total
      pct  = total.positive? ? ((used.to_f / total) * 100).round : 0
    ...
    def self.print_health(health, az = nil)
      ...
      print_cpu(health)
      print_thermal(health)
      puts
      print_memory(health)
      print_sfps(health, az)
    end

    def self.print_cpu(health)
      row "CPU temp", health.cpu_temperature ? temp_str(health.cpu_temperature) : nil

      if (load = health.load_averages)
        load_color = if load[0] >= 8
                       "\e[31m"
                     else
                       (load[0] >= 4 ? "\e[33m" : "")
                     end
        puts "  #{DIM}#{"CPU load".ljust(16)}#{RESET}  #{load_color}#{load[0]} / #{load[1]} / #{load[2]}  (1m / 5m / 15m)#{RESET}"
      end

      return unless (loading = health.cpu_loading)

      puts "  #{DIM}#{"CPU usage".ljust(16)}#{RESET}  #{loading["busy"].round(1)}% busy  (#{loading["usr"].round(1)}% user, #{loading["sys"].round(1)}% sys)"
    end

    def self.print_thermal(health)
      sensors = health.thermal_sensors
      ...
      puts "  #{DIM}#{"-" * 38}#{RESET}"
      sensors.each do |s|
        fan_rpm = if s.fan_speed_rpm
                    "#{s.fan_speed_rpm} RPM"
                  else
                    (s.fan_pct ? "passive" : "off")
                  end
        fan_pct = s.fan_pct ? "#{s.fan_pct}%" : "—"
        puts "  Sensor #{s.id}  #{temp_color(s.temperature_c)}#{temp_str(s.temperature_c).rjust(7)}#{RESET}  #{fan_pct.rjust(6)}  #{fan_rpm}"
      end
    end

    def self.print_memory(health)
      ...
      if total
        ...
        avail_color  = if avail_pct  && avail_pct < 15
                         "\e[31m"
                       else
                         (avail_pct && avail_pct < 25 ? "\e[33m" : "")
                       end
        commit_color = if commit_pct && commit_pct > 150
                         "\e[31m"
                       else
                         (commit_pct && commit_pct > 100 ? "\e[33m" : "")
                       end

        row "RAM total", bytes_human(total)
        puts "  #{DIM}#{"RAM available".ljust(16)}#{RESET}  #{avail_color}#{bytes_human(avail)}#{" (#{avail_pct}%)" if avail_pct}#{RESET}" if avail
        if committed
          puts "  #{DIM}#{"RAM committed".ljust(16)}#{RESET}  #{commit_color}#{bytes_human(committed)}#{" (#{commit_pct}% of total)" if commit_pct}#{RESET}"
        end
      end
      ...
      if pressure.any?
        ...
        pressure.each { |e| puts "  #{DIM}#{e.name.ljust(36)}#{RESET}  failcnt=#{e.mem_failcnt}  oom_kills=#{e.oom_kills}" }
      end
      ...
      puts "  #{DIM}#{"-" * 68}#{RESET}"
      consumers.each do |e|
        puts "  #{e.name.ljust(36)}  #{bytes_human(e.mem_bytes).rjust(7)}  #{bytes_human(e.swap_bytes).rjust(7)}  #{bytes_human(e.mem_bytes + e.swap_bytes)}"
      end
    end

    def self.print_sfps(health, az = nil)
      sfps = health.sfp_modules
      return unless sfps.any?

      puts
      sfps.each do |sfp|
        link_str = if sfp.link_up == true
                     "\e[32mlinked#{RESET} @ #{sfp.speed_mbps}Mb/s"
        ...
        row "  Serial", az ? az.serial(sfp.serial) : sfp.serial
    ...
    def self.print_boot_history(hist, from: nil, to: nil, az: nil)
      ...
      puts "  #{BOLD}#{filtered} boots#{" total" unless from || to}#{RESET} — " \
    ...
    def self.print_reboot_rate_by_firmware(boots)
      ...
      groups.each do |g|
        ...
        days  = ((g[:to] - g[:from]) / 86_400.0).round(1)
        rate  = days.positive? ? (days / [g[:count], 1].max).round(1) : "—"
        rate_str = g[:count].positive? ? "1 per #{rate}d" : "none"
        color_str = g[:count].positive? ? color(:improper) : color(:normal)
    ...
    def self.bytes_human(bytes)
      ...
      val == val.to_i ? "#{val.to_i} #{units[exp]}" : "#{format("%.1f", val)} #{units[exp]}"
    end
